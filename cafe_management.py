...
class BakeryOrderManagement:
    def __init__(self):
        ...
        self.items = ["Bread", "Croissant", "Muffin", "Bagel", "Cake" ]  # List of available items

    def add_order(self):
        customer_name = input("Enter customer name: ")
        print("Available items:", ", ".join(self.items))
        item = input("Enter item ordered: ")
        if item not in self.items:
            print(f"Item not available. Please choose from: {', '.join(self.items)}")
            return
